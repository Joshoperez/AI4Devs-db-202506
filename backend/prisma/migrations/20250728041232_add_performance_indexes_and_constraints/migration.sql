...
-- Add CHECK constraint for education dates (start <= end) - commented out due to potential issues
-- ALTER TABLE education ADD CONSTRAINT education_date_check CHECK (end_date IS NULL OR start_date <= end_date);

-- Add CHECK constraint for work experience dates (start <= end) - commented out due to potential issues
